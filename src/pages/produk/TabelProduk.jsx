...
export default function TabelProduk({ dataProduk, edit, deleteProduk }) {
  const headers = ["NO", "ID", "NAMA PRODUK", "HARGA BELI", "HARGA JUAL", "STOK PRODUK", "SATUAN", "AKSI"];
        ...
        dataProduk.map((item, i) => (
          <tbody key={i} >
            <tr>
              ...
              <td className="p-2 border text-end border-gray-300">{item.harga_beli.toLocaleString()}</td>
              <td className="p-2 border text-end border-gray-300">{item.harga_jual.toLocaleString()}</td>
              <td className="p-2 border text-end border-gray-300">{item.stok.toLocaleString()}</td>
              <td className="p-2 border border-gray-300 capitalize">{item.satuan}</td>
